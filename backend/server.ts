...
app.post('/api/auth/register/customer', async (req, res) => {
  ...
  try {
    ...
    await client.query(
      ...
      [user_id, email.toLowerCase(), password, 'customer', sanitizedFirstName, sanitizedLastName, phone_number, now, 'active', false, email_verification_token, now, now]
    );
    ...
    if (default_delivery_address && default_delivery_address.street_address) {
      ...
      await client.query(
        ...
        [address_id, user_id, `${sanitizedFirstName} ${sanitizedLastName}`, phone_number, default_delivery_address.street_address, default_delivery_address.city || '', default_delivery_address.state || '', default_delivery_address.postal_code || '', 'USA', true, now, now]
      );
...
app.get('/api/admins/me', authenticateToken, requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
...
app.get('/api/admin/me', authenticateToken, requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
    const result = await pool.query('SELECT * FROM admins WHERE user_id = $1', [req.user.user_id]);
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'AdminNotFound', message: 'Admin profile not found' });
...
app.get('/api/suppliers/me/delivery/settings', authenticateToken, requireSupplier, async (req: AuthRequest, res: Response) => {
  try {
    const supplier_id = req.user.supplier_id;
    
    // Get delivery zones
    const zonesResult = await pool.query(
      'SELECT * FROM delivery_zones WHERE supplier_id = $1 ORDER BY created_at DESC',
      [supplier_id]
    );
    ...
    const carrier_integrations = [
      {
        carrier_name: 'FedEx',
        integration_status: 'inactive' as const,
        api_key_status: 'not_configured' as const,
        last_sync: null,
...
app.put('/api/suppliers/me/delivery/zones', authenticateToken, requireSupplier, async (req: AuthRequest, res: Response) => {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    ...
    for (const zone of delivery_zones) {
      await client.query(
        'INSERT INTO delivery_zones (zone_id, supplier_id, zone_name, postal_codes, radius_miles, center_latitude, center_longitude, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)',
        [
          zone.zone_id || uuidv4(),
          supplier_id,
          zone.zone_name,
...
app.get('/api/suppliers/me/deliveries', authenticateToken, requireSupplier, async (req: AuthRequest, res: Response) => {
  try {
    const supplier_id = req.user.supplier_id;
...
app.put('/api/suppliers/me/delivery/carriers/:carrierName', authenticateToken, requireSupplier, async (req: AuthRequest, res: Response) => {
  try {
...
app.post('/api/promotions', authenticateToken, requireSupplier, async (req: AuthRequest, res: Response) => {
  try {
    const {
      supplier_id,
      promotion_name,
...
app.get('/api/admin/users', authenticateToken, requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const enrichedUsers = await Promise.all(result.rows.map(async (user: any) => {
      const enrichedUser = { ...user };
      
      if (user.user_type === 'customer') {
        const customerResult = await pool.query(
          'SELECT customer_id, account_type, trade_credit_status, trade_credit_limit, trade_credit_balance, trade_credit_used FROM customers WHERE user_id = $1',
          [user.user_id]
        );
        if (customerResult.rows.length > 0) {
        ...
        const supplierResult = await pool.query(
          'SELECT supplier_id, business_name, verification_status, rating_average, total_reviews, total_orders, total_sales FROM suppliers WHERE user_id = $1',
          [user.user_id]
        );
        if (supplierResult.rows.length > 0) {
...
app.get('/api/surplus/:listing_id', async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT sl.*, c.category_name, u.first_name || ' ' || u.last_name as seller_name, u.profile_photo_url as seller_photo
       FROM surplus_listings sl
       INNER JOIN categories c ON sl.category_id = c.category_id
       INNER JOIN customers cu ON sl.seller_id = cu.customer_id
       INNER JOIN users u ON cu.user_id = u.user_id
    ...
    const now = new Date().toISOString();
    await pool.query(
      'UPDATE surplus_listings SET views_count = views_count + 1, updated_at = $1 WHERE listing_id = $2',
      [now, req.params.listing_id]
    );
...
app.patch('/api/surplus/:listing_id', authenticateToken, requireCustomer, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (original_price !== undefined) { updates.push(`original_price = $${paramCount++}`); values.push(original_price); }
    if (price_type !== undefined) { updates.push(`price_type = $${paramCount++}`); values.push(price_type); }
    if (quantity !== undefined) { updates.push(`quantity = $${paramCount++}`); values.push(quantity); }
    if (pickup_location !== undefined) { updates.push(`pickup_location = $${paramCount++}`); values.push(pickup_location); }
    if (pickup_instructions !== undefined) { updates.push(`pickup_instructions = $${paramCount++}`); values.push(pickup_instructions); }
    if (shipping_available !== undefined) { updates.push(`shipping_available = $${paramCount++}`); values.push(shipping_available); }
    if (shipping_rate !== undefined) { updates.push(`shipping_rate = $${paramCount++}`); values.push(shipping_rate); }
    if (status !== undefined) { updates.push(`status = $${paramCount++}`); values.push(status); }
    if (reason_for_selling !== undefined) { updates.push(`reason_for_selling = $${paramCount++}`); values.push(reason_for_selling); }
...
app.delete('/api/surplus/:listing_id', authenticateToken, requireCustomer, async (req: AuthRequest, res: Response) => {
  try {
    const result = await pool.query(
      'DELETE FROM surplus_listings WHERE listing_id = $1 AND seller_id = $2 RETURNING listing_id',
      [req.params.listing_id, req.user.customer_id]
    );
...
app.get('/api/surplus/:listing_id/offers', authenticateToken, requireCustomer, async (req: AuthRequest, res: Response) => {
  try {
    const result = await pool.query(
      `SELECT so.*, u.first_name || ' ' || u.last_name as buyer_name, u.profile_photo_url as buyer_photo
...
app.post('/api/surplus/:listing_id/offers', authenticateToken, requireCustomer, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const listingResult = await pool.query(
      'SELECT listing_id, seller_id, status FROM surplus_listings WHERE listing_id = $1',
      [req.params.listing_id]
    );
    ...
    const result = await pool.query(
      ...
      RETURNING *`,
      [offer_id, req.params.listing_id, req.user.customer_id, offer_amount, message || null, 'pending', now, now, now]
    );
...
// Update offer status
app.patch('/api/surplus/offers/:offer_id', authenticateToken, requireCustomer, async (req: AuthRequest, res: Response) => {
  try {
