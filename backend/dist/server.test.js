...
let orderCustomerToken;
let orderCustomerId;
let orderAddressId;
// Review test variables (used across multiple test suites)
let reviewOrderId;
let reviewSupplierId;
let reviewProductId;
let reviewCustomerToken;
    ...
    describe('GET /api/products - Search & Filter', () => {
      ...
      `, [testSupplierId]);
        ...
        it('should sort products by price ascending', async () => {
            ...
            expect(response.status).toBe(200);
            const prices = response.body.products.map((p) => p.price_per_unit);
      ...
      `, ['prod_nonsearch', testSupplierId, 'cat_lumber', 'NONSEARCH', 'Hidden Product', 99.99, 'piece', 50, 10, 'active', false, 1, 1, 'all']);
            const response = await request(app)
                .get('/api/products');
            expect(response.status).toBe(200);
            const hiddenProduct = response.body.products.find((p) => p.product_id === 'prod_nonsearch');
...
describe('Shopping Cart Endpoints', () => {
    ...
    describe('GET /api/cart - Get Cart', () => {
        it('should return cart with all items and calculations', async () => {
            ...
            const expectedSubtotal = response.body.items.reduce((sum, item) => {
...
describe('Admin Endpoints', () => {
    describe('GET /api/admin/users - Search Users', () => {
        ...
        it('should search users by email query', async () => {
            ...
            const foundAdmin = response.body.users.find((u) => u.email === 'admin@buildeasy.com');
...
describe('End-to-End User Journeys', () => {
    ...
    it('should handle multi-supplier order correctly', async () => {
        ...
        const uniqueSuppliers = new Set(cartRes.body.items.map((item) => item.supplier_id));
...
describe('Performance & Load Tests', () => {
    ...
    it('should handle multiple concurrent requests', async () => {
        const requests = Array(20).fill(null).map((_, i) => request(app)
            .get('/api/products')
...
describe('Business Logic & Calculations', () => {
    ...
    `, ['bulk_price_prod', supplier.rows[0].supplier_id, 'cat_lumber', 'BULK-SKU', 'Bulk Pricing Product', 100.00, 'piece', JSON.stringify({ '10+': 90.00, '50+': 80.00, '100+': 75.00 }), 500, 50, 'active', 1, 1, 'all', true]);
        ...
        if (quantity >= 100)
            applicablePrice = bulkPrices['100+'];
        else if (quantity >= 50)
            applicablePrice = bulkPrices['50+'];
        else if (quantity >= 10)
            applicablePrice = bulkPrices['10+'];
    ...
    it('should calculate delivery fees based on zones', async () => {
        ...
        const distance = Math.sqrt(Math.pow(customerLocation.latitude - supplierLocation.latitude, 2) +
            Math.pow(customerLocation.longitude - supplierLocation.longitude, 2)) * 69; // Rough miles
...
describe('Input Validation & Security', () => {
    ...
    it('should enforce rate limiting on auth endpoints', async () => {
        ...
        const attempts = Array(10).fill(null).map(() => request(app)
            .post('/api/auth/login')
            .send({
...
describe('Edge Cases & Boundary Conditions', () => {
    ...
    it('should handle concurrent stock updates correctly', async () => {
        ...
        const updates = Array(5).fill(null).map((_, i) => pool.query('UPDATE products SET stock_quantity = stock_quantity - $1 WHERE product_id = $2', [10, productId]));
